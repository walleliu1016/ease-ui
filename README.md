...
- **双向 CLI 包装** — App 内发送 prompt，通过 stream-json 协议流式接收文本 / tool use / permission request
- **拦截 PermissionRequest** — App 内弹面板批准 / 拒绝；可配置 auto-allow bash
- **编辑 Hooks 配置** — 表单化 `~/.claude/settings.json` 的 hooks 段，自动备份
- **打开系统终端** — 一键 `claude -r <session-id>`（macOS Terminal.app / iTerm / Windows Terminal / xterm）
...
| 持久化 | 本地 JSON + bcrypt | v1 不上 DB；`~/.ease-ui/` 三个文件搞定 |
...
│      ├── jsonl         read ~/.claude/projects/*/<sid>.jsonl (history, no write)             │
│      ├── hooks         PermissionRequest handler + settings.json editor (with backup)        │
│      ├── terminal      open external terminal w/ `claude -r <sid>`                            │
│      ├── settings      ~/.ease-ui/config.json                                                │
│      ├── events        process-local pub/sub bus                                             │
│      ├── log           lumberjack rotating logger                                            │
│      └── cli           `ease-ui init` subcommand                                             │
│                                                                                               │
...
└──────────────────────────────────────────────────────────────────────────────────────────────┘
            ...
            ├── config.json (write)          └── projects/*/<sid>.jsonl  (read-only)
            └── logs/*.log  (write)
```
...
7. 状态机用 `wailsruntime.EventsEmit(ctx, "session:event", payload)` 推给前端
8. 前端 `useEventStream` 订阅 `session:event`，更新 Pinia store
...
```
ease-ui/
├── main.go                      # 入口：dispatch `ease-ui init` vs GUI
├── app.go                       # Wails Run 配置：标题、尺寸、AssetServer、OnStartup、Bind
│
├── internal/                    # 所有后端代码（不导出）
...
│   │   ├── views/               # LoginView / HomeView / SettingsView / UninitView
...
```bash
# 1) 初始化密码（v1 不在 GUI 内引导）
./build/bin/ease-ui.app/Contents/MacOS/ease-ui init
# 等价：./ease-ui init（如果当前目录有编译好的二进制）
...
# 3) 重置密码
rm ~/.ease-ui/auth.json && ./ease-ui init
```
...
| 文件 | 路径 | 权限 |
|---|---|---|
| Ease 配置 | `~/.ease-ui/config.json` | 读写 |
| 鉴权（bcrypt hash） | `~/.ease-ui/auth.json` | 读写 |
| 日志（lumberjack 滚动） | `~/.ease-ui/logs/*.log` | 读写 |
| Claude Hooks（可写） | `~/.claude/settings.json` | 读写（编辑前自动 .bak） |
| Claude 会话（只读） | `~/.claude/projects/*/<sid>.jsonl` | 只读 |
...
- 🐛 **macOS 首次 `ease-ui init` 时 TCC 会弹"终端想控制 XXX"** —— Terminal / iTerm 没在系统偏好里授权。无功能影响，烦人。
...
- ❌ 内嵌 chat markdown 渲染 —— 用 CLI 的流式输出已经够清楚
- ❌ 多账号切换 —— v1 一个本地账户
