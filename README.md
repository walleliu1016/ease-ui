...
- **打开系统终端** — 一键 `claude -r <session-id>`（macOS iTerm2 / Terminal.app；Windows Windows Terminal / cmd；Linux gnome-terminal / konsole / xterm）
- **终端模式** — 外部终端有 PreToolUse 等活动时，Composer 自动锁住并提示「会话已在外部终端中打开」
