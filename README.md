...
- **Session ID 由 Claude 生成** — Ease UI 不再自己 newID。`CreateSession` 启 stream-json 进程 + 阻塞等 SessionStart hook 返回 Claude 生成的真实 UUID（15s 超时），UI 期间显示加载动画
...
- **HTTP API 写入 session** — `POST /api/send` 接收 `{"session_id":"...","prompt":"..."}`，转发到对应 session 的 claude stdin。供外部脚本 / 集成测试调用
...
- **Composer 按 session 隔离** — Pinia store 里 `drafts: Record<sid, string>` 保留每个 session 独立的未发送草稿，切换 session 不丢字
- **AdoptSession** — 启动 Ease UI 之前已存在的历史 session（jsonl 里有但 a.sessions map 没注册），第一次发消息时按需 `AdoptSession` 拉起 stream-json 进程接管；已在 a.sessions 中的 sid 走幂等 noop
...
1. 用户在 `Composer.vue` 输入 → `useSessionsStore.send(id, prompt)`
2. Store 调 `useWails.SendMessage(sid, prompt)` → `window.go.app.App.SendMessage(...)`
3. Wails runtime 序列化成 JSON-RPC 消息，XPC 送到 Go 进程
4. `internal/app` 的 `SendMessage` 查 session；如果是 App 模式，调用 `session.Session.Send(prompt)`，内部用 `envelopeUserMessage` 包成 `{"type":"user","message":{"role":"user","content":prompt}}` 写 stdin（**v1 关键：必须 envelope，裸文本会被 stream-json 解析器直接丢弃**）
5. Claude CLI 写出 stream-json 事件（`assistant` / `user` / `tool_use` / `permission_request` / `result`），Go 端 `protocol.Parser` 按行解析
6. 每个事件经 `events.Bus` 派发到 `internal/app` 的 session 状态机
7. 状态机用 `wailsruntime.EventsEmit(ctx, "session:<id>", payload)` 推给前端
8. 前端 `useEventStream` 订阅 `session:<id>`，更新 Pinia store；前端 `handleEvent` 按 `msgId` 去重，相同 assistant 文本不会重复渲染
9. 进程退出（`done` 事件）时，前端 `reloadFromJsonl(sid)` 从 jsonl 重新加载最后 PAGE_SIZE 条作为兜底（处理 stdout 管道无数据 / 不完整的边界 case）
10. Vue 响应式系统自动重渲染 `MessageBubble` / `ToolUseBlock` / `PermissionPanel`

### 数据流：新建 session（Claude 生成 UUID）

1. 前端 `useSessionsStore.create(workdir, prompt)` → `App.CreateSession(workdir, prompt)`
2. Go 端用 `process.ModeAuto` 启动 `claude --input-format stream-json --output-format stream-json`（**不带** `--session-id` / `--resume` flag —— Claude 自己生成）
...
6. Go 端 `CreateSession` 收到 UUID → 用真实 ID 建 `session.Session` + 注册到 `a.sessions` + `pumpEvents` + 写首条 prompt
...
1. hookserver `/api/send` 收到 POST → 调 `App.SendMessage(sid, prompt)`（注册在 `hookSrv.OnSend`）
2. Go 端逻辑跟前端发送完全相同：envelope 写 stdin → stream-json 流回前端
3. 失败返回 `{"ok":false,"error":"..."}`，成功返回 `{"ok":true}`
4. 用途：CI / 集成测试 / 自定义脚本往 Ease UI 控制的 session 灌入 prompt
